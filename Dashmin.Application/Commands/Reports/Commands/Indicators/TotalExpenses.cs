...
namespace Dashmin.Application.Reports.Commands
{
    ...
    /// </summary>
    public class TotalExpenses : IRequest<Result>
    {
        /// <summary>
        ...
        public TotalExpenses(List<IndicatorResult> model,Organization organization)
        {
            _model = model;
        ...
        /// <summary>
        /// Clase que se encarga de realizar la actualizacion de la base de datos a petición de  <see cref="TotalExpenses"/>
        /// esta clase implementa la interfaz <see cref="IRequestHandle{TotalExpenses, Result}"/>
        /// </summary>
        class TotalExpensesHandeler : IRequestHandler<TotalExpenses, Result>
        {
            /// <summary>
            ...
            /// <summary>
            /// Constructor cuya funcion es la de crear una nueva instancia de <see cref="TotalExpensesHandeler"/>
            /// </summary>
            ...
            public TotalExpensesHandeler(IConnectionService connection,IApplicationDBContext context)
            {
            ...
            public async Task<Result> Handle(TotalExpenses request, CancellationToken cancellationToken)
            {
                ...
                {
                    List<var_total_egresos> DataSet = new List<var_total_egresos>();
                    int x = 0;
                    try
                    {
                        string sql = @$"INSERT INTO var_total_egresos (empresa_contable,fecha_dato,totalinternamientonormal,totalambulatorio,totalprepago,totalinternofueurgencias,totalinternofueambulatorio,totalreciennacido,totalinternamientoexpres,totalcortaestancia,totalinternofuecortaestancia,totalcortaestanciafueambulatorio,totalinternamientonormalconcentrado,organizacion_id,sexo,edad,fecha_genera_dato,clave_cuenta_paciente)
                        VALUES (@empresa_contable, @fecha_dato, @totalinternamientonormal, @totalambulatorio, @totalprepago, @totalinternofueurgencias, @totalinternofueambulatorio, @totalreciennacido, @totalinternamientoexpres, @totalcortaestancia, @totalinternofuecortaestancia, @totalcortaestanciafueambulatorio, @totalinternamientonormalconcentrado, @organizacion_id, @sexo, @edad, @fecha_genera_dato, @clave_cuenta_paciente);";
                        ...
                        foreach(IndicatorResult model in request._model)
                        {
                            try
                            {
                                var_total_egresos data = new var_total_egresos();
                                ...
                                data.totalambulatorio                    = Int32.Parse(model.Value.Split('|')[1] != "" ? model.Value.Split('|')[2] : "0");
                        ...
                        var deleteQuery = @$"DELETE FROM var_total_egresos WHERE organizacion_id = {organization.IdOrganization} AND fecha_genera_dato between '{DataSet.Min(o =>o.fecha_genera_dato).ToString("yyyy-MM-dd")}' and '{DataSet.Max(o =>o.fecha_genera_dato).ToString("yyyy-MM-dd")}'";
                        var affectedRows = conn.Execute( deleteQuery,commandType: CommandType.Text,commandTimeout: 900);

                        _= _context.var_total_egresos.AddRangeAsync(DataSet);
                        await _context.SaveChangesAsync(cancellationToken);

                        Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + $"{organization.IdOrganization} - {organization.Name} - var_total_egresos: Complete insert registers {totalCounter} \n");
                    }
                    catch (System.Exception ex)
                    {
                        Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + $"{organization.IdOrganization} - {organization.Name} - var_total_egresos: Error {ex.Message} {JsonSerializer.Serialize(request._model[x])} \n");
